...
export function GUBERAssistant() {
  ...
  const BRIEFING_SHOWN_KEY = "jac_briefing_shown_v1";
  ...
  useEffect(() => {
    ...
    try { if (sessionStorage.getItem(BRIEFING_SHOWN_KEY) === "1") return; } catch {}
    ...
    fetch("/api/jac/briefing")
      ...
      .then((data: { text: string | null; chips: Array<{ label: string; message: string }> }) => {
        ...
        setMessages(prev => {
          ...
          return [prev[0], { role: "assistant", content: data.text!, actions: data.chips || [] }];
        });
      })
      .catch(() => {});
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  return (
    ...
    >
      ...
      >
        ...
        {user && s.open && jacOpportunities && jacOpportunities.filter(o => o.type !== "pending_action").length > 0 && (
          <div
          ...
          >
            ...
            <div className="divide-y" style={{ borderColor: "hsl(222 47% 14%)" }}>
              {jacOpportunities.filter(o => o.type !== "pending_action").slice(0, 3).map((opp, i) => (
                <button
                  key={i}
