 ...
 *
 * Audio amplitude: real audio-driven when a live tap is available (ElevenLabs
 * ConvAI WebRTC stream or text-mode TTS pipeline, via jac-audio-analyser.ts),
 * with the original spring-physics simulation as fallback when no live audio
 * source is connected.
 */

import { useEffect, useRef, useCallback } from "react";
import { getJacLiveAmplitude, ensureJacAudioTapListener } from "@/lib/jac-audio-analyser";
...
export function JacAnimatedCharacter({ state, style, heightPx }: Props) {
  ...
  // Install the document-level tap that captures ElevenLabs' WebRTC <audio>
  // element so the mouth can follow the real voice. Idempotent.
  useEffect(() => { ensureJacAudioTapListener(); }, []);
  ...
  const draw = useCallback((ts: number) => {
    ...
    // Target amplitude: prefer REAL audio amplitude from the live analyser tap
    // (ElevenLabs ConvAI stream or text-mode TTS); fall back to the
    // multi-frequency speech simulation when no live audio is available.
    let target = 0;
    if (isSpeaking) {
      const live = getJacLiveAmplitude();
      if (live !== null) {
        target = live;
      } else {
        const raw =
