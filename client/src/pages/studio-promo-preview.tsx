  ...
  useEffect(() => {
    ...
    if (raw) {
      try {
        ...
        // New format: plain JSON (encodeURIComponent only, no btoa layer).
        // Fall back to base64 for any URL generated by the old client or
        // the server-side Playwright renderer before this fix was deployed.
        let parsed: unknown;
        try {
          parsed = JSON.parse(decoded);
        } catch {
          // Legacy path: decoded string is still base64-encoded JSON
          try {
            parsed = JSON.parse(atob(decoded));
          } catch {
            // Node Buffer-style UTF-8 base64 (server renderer legacy path)
            const bytes = Uint8Array.from(atob(decoded), (c) => c.charCodeAt(0));
            parsed = JSON.parse(new TextDecoder().decode(bytes));
          }
        }
        setData(parsed as PromoData);
      } catch (e) {
        console.error("[studio/preview] failed to parse promo data from URL:", e);
      }
    }
  }, []);
