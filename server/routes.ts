    ...
    try {
      ...
      if (stateResult.isNative) {
        ...
        if (pollKey) {
          ...
          // Build deep links so the "Return to GUBER" button can hand the JWT
          // directly to the app. We provide TWO formats and try both:
          //
          //   1. Android intent:// URL — the official Android mechanism for
          //      launching an app from a browser tab. Samsung Internet and
          //      Chrome Custom Tab both honor this from a user gesture, even
          //      when they would block a plain custom-scheme navigation.
          //      Includes the package name (com.guber.app) so Android knows
          //      exactly which app to open with no chooser dialog.
          //
          //   2. Plain guber:// URL — fallback for iOS / older Android
          //      browsers that don't understand intent:// syntax.
          //
...
<body>
  ...
  app.get("/api/marketplace/:id/buyer-order/pdf", async (req: Request, res: Response) => {
    try {
      ...
      const logoPath = `${process.cwd()}/client/public/icon-1024-transparent.png`;
      ...
      doc.fillColor(C_GACC).fontSize(7).font("Helvetica-Bold")
        .text("guberapp.app/biz", ML + 12, s5Y + 34);
      ...
      try { doc.image(logoPath, ML, footY + 8, { height: 38 }); } catch {}
      doc.fillColor(C_GRAY).fontSize(7).font("Helvetica").text("Generated by", ML + 46, footY + 10);
      doc.fillColor(C_BLK).fontSize(10).font("Helvetica-Bold").text("GUBER Marketplace", ML + 46, footY + 20);

      // Disclaimer text — full width now that QR is removed
      ...
      doc.fillColor(C_GRAY).fontSize(6).font("Helvetica")
        .text(discText, ML + 46, footY + 38, { width: IW - 50 });
...
<div class="page">
  ...
  async function runStudioGeneration(opts: {
    ...
    // upload and the studio_session_files row write destroys them — no
    // orphans in paid storage.
    ...
    const cleanup = createCloudinaryCleanup(cloudinary);
    ...
    try {
      ...
      const effectiveResourceType: "video" | "image" =
        provider.resourceType === "image" ? "image" : "video";
      try {
        if (cloudinary) {
          ...
          cleanup.track(cloudinaryPublicId, effectiveResourceType);
        }
      } catch (err: any) {
      ...
      // The asset is now persisted to studio_session_files and owned by the
      // user — drop it from the cleanup list IMMEDIATELY so any late failure
      // in touchStudioSession / logStudioGeneration doesn't destroy a file
      // we just handed over.
      cleanup.clear();
  ...
  app.post("/api/studio/generate/commercial", requireAuth, async (req: Request, res: Response) => {
    ...
    // leave orphans in paid storage (task-538). Lifted into a shared
    // helper in task-541 so wan_motion / kling / mirror_motion / music
    // get the same guarantee via runStudioGeneration().
    const { createCloudinaryCleanup } = await import("./studio/cloudinary-cleanup");
    let commercialCleanup = createCloudinaryCleanup(null);
    const cleanupCloudinaryAssets = async () => {
      await commercialCleanup.cleanup("commercial");
    };
    try {
      ...
      commercialCleanup = createCloudinaryCleanup(cloudinary);
      ...
      const reHost = async (sourceUrlOrDataUrl: string, resourceType: "video" | "image" | "raw", folder: string): Promise<{ url: string; publicId: string | null }> => {
        ...
        try {
          ...
          commercialCleanup.track(up.public_id, effectiveResourceType);
